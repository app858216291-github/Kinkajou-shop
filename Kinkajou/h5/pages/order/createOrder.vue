	...
	<view>
		...
		<view class="goods-section">
			<view class="g-header b-b">
				...
				<text class="name">兰花交易网</text>
			</view>
			<!-- 商品列表 -->
			<view v-for="(item, index) in productList" :key="index" class="g-item">
				<image :src="item.mainImg"></image>
				<view class="right">
					<text class="title clamp">{{item.title}}</text>
					<text class="spec">{{item.color}} {{item.size}}</text>
					<view class="price-box">
						<text class="price">￥{{item.price}}</text>			
						<text v-if="productid==0" class="number">x {{item.mount}}</text>
						<uni-number-box
							v-if="productid!=0"
							class="step"
							:min="1" 
		...
		methods: {
			...
			async onLoad(options){	
				...
				if(data==undefined){
					...
					// let goodsData=JSON.parse(options.data)
					// goodsData=eval("("+options.data+")");
					// this.productList=goodsData
					if (this.productid!=0 & this.productid!=undefined){
						this.productList=[await this.$shop.getProduct(this.$shop.prop().serviceUrl+'/product/productDetail?id='+this.productid,{prop:options.prop})]
						this.productList[0].mount=1;
						this.productList[0].number=1;
						this.productList[0].propid=options.prop
						
					}else{
						this.productList=await this.$shop.getProductList(this.$shop.prop().serviceUrl+"/product/productList",{});
					}
					...
					for(let i=0;i<data.goodsData.length;i++){
						ids=ids+data.goodsData[i].productid+','
					}
					ids=ids.substring(0,ids.length-1)
					
					// let cc=data.goodsData
					let carProducts=await this.$api.get(this.$shop.prop().serviceUrl+'/product/carList?productids='+ids,"")
					carProducts=carProducts.data
					for(let i=0;i<carProducts.length;i++){
						// let p=;
						
						let mainImgs=carProducts[i].main_image
						if(mainImgs[0]==','){
			...
			numberChange(data) {
				...
				this.productList[data.index].number=data.number;
				this.productList[data.index].mount=data.number;
			...
			async submit(){
				...
				let mount=order.mount;
				console.info("移除购物车产品")
				for(let i=0;i<this.productList.length;i++){
					this.$api.get(this.$shop.prop().serviceUrl+"/product/removeCar?productid="+this.productList[i].id,"")
				}
				uni.redirectTo({
					url: `/pages/money/pay?orderNo=${orderNo}&total=${total}&orderid=${orderid}`
				})
			},
			stopPrevent(){}
		}

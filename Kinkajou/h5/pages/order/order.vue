					...
					>
						...
						<scroll-view v-if="item.products.length > 1" class="goods-box" scroll-x>
							<view
								...
								class="goods-item" @click="navToDetail(item)"
							>
							<image class="goods-img" :src="goodsItem.mainImg" mode="aspectFill"></image>
							</view>
						</scroll-view>
						<view 
							v-if="item.products.length == 1" 
							class="goods-box-single"
							v-for="(goodsItem, goodsIndex) in item.products" :key="goodsIndex"  @click="navToDetail(item)"
						>
							<image class="goods-img" :src="goodsItem.mainImg" mode="aspectFill"></image>
							<view class="right">
								<text class="title clamp">{{goodsItem.title}}</text>
								<text class="attr-box">{{goodsItem.price}}  x {{goodsItem.mount}}</text>
								<text class="price">{{goodsItem.mount}}</text>
							</view>
						</view>
						...
						<!-- <view class="action-box b-t" v-if="item.orderStatus != 9">
							<button class="action-btn" @click="cancelOrder(item)">取消订单</button>
							<button class="action-btn recom" @click="pay(item)">查看2{{item.orderStatus}}进度</button>
						</view> -->
					</view>
	...
	export default {
		...
		methods: {
			...
			async loadData(source){
				...
				// orders=orders.filter(item=>item.orderStatus==1);
			...
			pay(item){
				...
				let orderId=item.id
				uni.redirectTo({
					url: `/pages/money/pay?orderNo=${orderNo}&mount=${mount}&orderid=${orderId}`
				})
			},
			//收货操作
			receivce(item){
				...
				this.$api.get(this.$shop.prop().serviceUrl+"/order/changeStatus?orderId="+item.id+"&orderStatus=3",{})
				uni.redirectTo({
			...
			cancelOrder(item){
				...
				setTimeout(()=>{
					...
					//后台关闭订单
					_self.$api.get(_self.$shop.prop().serviceUrl+"/order/changeStatus?orderId="+item.id+"&orderStatus=9",{});
					uni.hideLoading();
				}, 600)
			},
